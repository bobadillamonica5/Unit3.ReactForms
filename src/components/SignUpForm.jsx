...
export default function SignUpForm( { setToken } ) {
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            const obj = {
                username: username.length >= 8 ? username : setErrorChars('Minimum 8 characters required.'),
                password: password.includes(0) || password.includes(1) ? password: setErrorPw('Password must contain a 0 or 1.')
            }
            ...
            <div>
                ...
                {error && <p>{error}</p>}
                    <form onSubmit={handleSubmit} action="submit">
                        ...
                        <label>password:
                            <input value={password} onChange={(e) => {setPassword(e.target.value)}} type="text" name="password" id="" />
                        </label>
